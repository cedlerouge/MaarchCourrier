...
class ShippingTemplateController
{
    ...
    public function getById(Request $request, Response $response, array $args): Response
    {
        ...
        $shippingInfo['entities'] = !empty($shippingInfo['entities']) ? json_decode(
            $shippingInfo['entities'],
            true
        ) : [];

        $shippingInfo['subscriptions'] = !empty($shippingInfo['subscriptions']) ? json_decode(
            $shippingInfo['subscriptions'],
            true
        ) : [];
    ...
    public function delete(Request $request, Response $response, array $args): Response
    {
        ...
        $shippingInfo = ShippingTemplateModel::getById(
            ['id' => $args['id'], 'select' => ['label', 'subscriptions', 'account']]
        );
        if (empty($shippingInfo)) {
            return $response->withStatus(400)->withJson(['errors' => 'Shipping does not exist']);
        }
        ...
        $shippings = ShippingTemplateModel::get(
            ['select' => ['id', 'label', 'description', 'options', 'fee', 'entities']]
        );
    ...
    public function receiveNotification(Request $request, Response $response, array $args): Response
    {
        ...
        } elseif (!Validator::stringType()->in(
        ...
        } elseif (!empty($body['event_location']) && !Validator::equals('FR')->validate(
                $body['event_location']
            )) {
        ...
        foreach ($actionParameters as $phaseParameters) {
            if (
                !Validator::each(
                    Validator::in(array_keys(ShippingTemplateController::MAILEVA_EVENT_RESOURCES))
        ...
        if ($body['eventType'] == 'ON_STATUS_ARCHIVED') {
            ...
            if (!in_array('shipping_deposit_proof', array_column($previousAttachments, 'attachment_type'))) {
                ...
                if ($curlResponse['code'] != 200) {
                    $attachmentErrors[] = 'shipping_deposit_proof:download: deposit proof failed to download for sending ' .
                        json_encode(
                            [
                    ...
                    $attachmentId = StoreController::storeAttachment([
                        'title'       => _SHIPPING_ATTACH_DEPOSIT_PROOF . '_' .
                    ...
                    if (!empty($attachmentId['errors'])) {
                        $attachmentErrors[] = 'shipping_deposit_proof:save: could not save deposit proof to docserver: ' .
    ...
    public static function calculShippingFee(array $args)
    {
        ...
        foreach ($args['resources'] as $value) {
            ...
            $docserver = DocserverModel::getByDocserverId(
                ['docserverId' => $convertedResource['docserver_id'], 'select' => ['path_template']]
            );
            $pathToDocument = $docserver['path_template'] .
                str_replace('#', DIRECTORY_SEPARATOR, $convertedResource['path']) . $convertedResource['filename'];
    ...
    private static function checkToken($args): array
    {
        ...
        if (!Validator::notEmpty()->stringType()->equals('MaarchCourrier')->validate($payload['iss'])) {
            ...
            return ['errors' => 'Authentication failed'];
        } elseif (!Validator::notEmpty()->stringType()->equals($args['shippingApiDomainName'])->validate(
            $payload['aud']
        )) {
            return ['errors' => 'Authentication failed'];
        } elseif (!Validator::notEmpty()->intVal()->min($args['minIAT'])->max($now)->validate($payload['iat'])) {
            return ['errors' => 'Authentication failed'];
        } elseif (!Validator::notEmpty()->intVal()->equals($args['shippingTemplateId'])->validate(
    ...
    private static function subscribeToNotifications(array $shippingTemplate): array
    {
        ...
        $jwtAndIAT = ShippingTemplateController::generateToken(
            ['mailevaUri' => $mailevaConfig['uri'], 'shippingTemplateId' => $shippingTemplate['id']]
        );
        $jwt = $jwtAndIAT['jwt'];
        $iat = $jwtAndIAT['iat'];
            ...
            foreach ($resourceTypes as $resourceType) {
                $curlResponse = CurlModel::exec([
                    ...
                    'body'       => json_encode([
                        ...
                        'callback_url'  => $maarchUrl . '/rest/administration/shippings/' . $shippingTemplate['id'] .
                            '/notifications?auth_token=' . $jwt
                    ])
                ]);
                if ($curlResponse['code'] != 201) {
                    return [
                        'errors' => 'Maileva POST/subscriptions returned HTTP ' . $curlResponse['code'] . '; ' .
        ...
        $subscriptions = array_values(
            array_unique(array_merge(($shippingTemplate['subscriptions'] ?? []), $subscriptions))
        ) ?? [];
            ...
            if ($curlResponse['code'] != 204) {
                return [
                    'errors' => $curlResponse['response'] ??
                        ('Maileva DELETE/subscriptions/' . $subscriptionId . ' returned HTTP ' . $curlResponse['code'])
                ];
            }
        }
